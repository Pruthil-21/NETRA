...
from ..schemas import VehicleTraceSighting
from ..services import detections_service

router = APIRouter(prefix="/vehicle-traces", tags=["vehicle-traces"])


@router.get("/{plate_number}", response_model=list[VehicleTraceSighting])
def get_vehicle_trace(
    ...
    return detections_service.get_vehicle_trace(db, plate_number, scenario_run_id)
